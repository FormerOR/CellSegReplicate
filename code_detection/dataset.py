...
class DataFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #         print(f'sample[{i}] is a numpy array with shape {s.shape}')
        #     elif isinstance(s, Image.Image):
        #         print(f'sample[{i}] is a PIL Image with size {s.size}')
        #     else:
        #         print(f'sample[{i}] is of type {type(s)}')
        ...
        #     if isinstance(s, np.ndarray):
        #         arr = s
        #     elif hasattr(s, 'mode'):  # PIL.Image
        #         arr = np.array(s)
        #     if arr is not None and np.isnan(arr).any():
        #         print(f'NaN found in sample[{i}] at index {index}, path: {img_paths[i]}')
