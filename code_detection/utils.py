...
def compute_accuracy(pred, gt, radius, return_distance=False):
    ...
    gt_points = [region.centroid for region in gt_regions]
    gt_areas = [region.area for region in gt_regions]
    gt_points = np.array(gt_points)
    Ng = gt_points.shape[0]
    avg_gt_area = np.mean(gt_areas) if gt_areas else 0

    print('=> {:d} predicted points, {:d} ground truth points'.format(Np, Ng))
    print('=> Avg pred area: {:.2f}, Avg gt area: {:.2f}'.format(avg_pred_area, avg_gt_area))
